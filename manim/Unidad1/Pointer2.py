...
class PunteroAEntero(Scene):
    def construct(self):
        ...
        for i, val in enumerate(arreglo):
            ...
            if i == 1:
                value = Text(str(5), color=WHITE).scale(0.4)
                value.move_to(box.get_center())
                group = VGroup(box, value)
                vertical_boxes.add(group)
            elif i == 2:
                value = Text(str(10), color=WHITE).scale(0.4)
                value.move_to(box.get_center())
                group = VGroup(box, value)
                vertical_boxes.add(group)
            elif i == 5:
        ...
        elements = VGroup()
        elements.add(VGroup(Text('a', color=flecha_color).scale(0.4)))
        elements.add(VGroup(Text('b', color=flecha_color).scale(0.4)))
        ...
        for i, group in enumerate(vertical_boxes):
            if i==1:
                ...
                elements[2].next_to(group, LEFT, buff=0.3)
